...
NC_Handler NC_Open4Copy(NC_Handler pNcInfo,const char *file)
{
    if(pNcInfo!=NULL&&file!=NULL)
    {
        ...
        for(;i<pNcInfo->var_count;++i)
        {
            ...
            for(j=0;j<nc->vars[i].dim_count;++j)
            {
            ...
            for(j=0;j<pNcInfo->vars[i].attr_count;++j)
            {
...
int copyVariablesByUlimitDim(NC_Handler onc,NC_Handler nc,int start,int offset)
{
    if(onc!=NULL&&nc!=NULL&&start>=0&&offset>0)
    {
        ...
        for(;i<nc->var_count;++i)
        {
            ...
            nc->vars[i].count=(size_t*)malloc(sizeof(size_t)*nc->vars
    [i].dim_count);
            int udidx=-1;
            for(j=0;j<nc->vars[i].dim_count;++j)
            {
...
NC_DimInfo* NC_GetDimByName(struct _var_Info* var,const char const *dimName)
{
	...
	for(;i<var->dim_count;++i)
	{
	    if(!strncmp((var->pDims+i)->name,dimName,strlen((var->pDims+i)->name)))
		return var->pDims+i;
	}
    }
    return pdim;
...
{
    char result=-1;
    if(pDimInfo&&pNcInfo&&(strlen(pDimInfo->name)>0||pDimInfo->id>=0))
    {
        int err=NC_NOERR;
        if(strlen(pDimInfo->name)>0)
...
char NC_ReadFloatArray(NC_Handler pNcInfo,NC_Variable pVarInfo,double *pfVars)
{
    ...
    if(pNcInfo&&pVarInfo&&pfVars)
    {
        ...
        i=NC_NOERR;
        i=nc_get_vara_double(pNcInfo->ncid,pVarInfo->id,pVarInfo->start,pVarInfo->count,pfVars);
        if(NC_NOERR!=i)
            print_nc_error1(i,"Can not read values for variable(double)",pVarInfo->name);
        else
            result=0;
    }
    return result;
...
static char NC_ReadArray(NC_Handler pNcInfo,NC_Variable pVarInfo,void *pfVars)
{
	...
	for(i=0;i<pVarInfo->dim_count;++i)
        ...
        i=NC_NOERR;
	i=nc_get_vara(pNcInfo->ncid,pVarInfo->id,pVarInfo->start,pVarInfo->count,pfVars);
        if(NC_NOERR!=i)
            print_nc_error1(i,"Can not read values for variable",pVarInfo->name);
        else
            result=0;
    }
    return result;
